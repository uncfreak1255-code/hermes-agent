...
def recompute_ready(
    ...
    Parentless tasks parked by a current ``gave_up`` breaker event stay
    blocked until a human unblocks or retries them. Without that guard,
    the dispatcher can fall into ``gave_up`` -> ``promoted`` -> respawn loops.
    """
